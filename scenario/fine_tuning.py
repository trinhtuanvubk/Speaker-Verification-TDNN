...
import scenario.train as t
import scenario.module as d2l
# import torch_directml
from nnet.loss.loss import AAMSoftmax
...
if __name__ == "__main__":
    ...
    t.init_logs()
    train_dict, test_dict, people_num = dataset.load_files(mode=mode, folder_num=people_num,
                                                                   file_num=data_per_people, k=1)
    train_dataset = dataset.MyDataset(data_dict=train_dict, people_num=people_num, train=True,
                                     mel=mel, noise=noise)
    test_dataset = dataset.MyDataset(data_dict=test_dict, people_num=people_num, train=False,
                                    mel=mel, noise=noise)
